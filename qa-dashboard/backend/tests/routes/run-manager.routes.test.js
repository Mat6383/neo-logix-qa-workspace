...
describe('POST /api/runs', () => {
  test('403 — sans X-Requested-With', async () => {
    const res = await request(app).post('/api/runs').send({ syncProjectId: 'neo-pilot', name: 'Run R14', caseIds: [1] });
...
describe('POST /api/runs/:runId/merge-preview', () => {
  test('403 — sans X-Requested-With', async () => {
    const res = await request(app).post('/api/runs/42/merge-preview').send({ caseIds: [1] });
    expect(res.status).toBe(403);
  });

  test('400 — runId invalide', async () => {
    const res = await request(app).post('/api/runs/abc/merge-preview').set(CSRF).send({ caseIds: [1] });
    expect(res.status).toBe(400);
  });
  ...
  test('200 — preview calculé', async () => {
    ...
    const res = await request(app).post('/api/runs/42/merge-preview').set(CSRF).send({ caseIds: [1, 2, 3] });
    expect(res.status).toBe(200);
...
describe('POST /api/runs/:runId/merge', () => {
  test('403 — sans X-Requested-With', async () => {
    const res = await request(app).post('/api/runs/42/merge').send({ caseIds: [1] });
    expect(res.status).toBe(403);
  });

  test('400 — runId invalide', async () => {
    const res = await request(app).post('/api/runs/0/merge').set(CSRF).send({ caseIds: [1] });
    expect(res.status).toBe(400);
  });

  test('200 — merge exécuté', async () => {
    mockRunManager.mergeRunCases.mockResolvedValue({ added: 2, preserved: 1 });
    const res = await request(app).post('/api/runs/42/merge').set(CSRF).send({ caseIds: [1, 2, 3] });
    expect(res.status).toBe(200);
