...
from database import (
    ...
    get_matrix_rows, add_matrix_row, update_row_impact, update_row_probability,
    update_row_impact_description, delete_row,
)
from gitlab_client import fetch_all_bug_issues, search_projects, fetch_issue_comments, extract_impact_score_from_comments
from data_processor import process_issues, enrich_row, IMPACT_LEVELS, PROBABILITY_LEVELS
...
def risk_matrix():
    prob_levels = [{"key": k, "label": l, "value": v} for k, l, v, _ in PROBABILITY_LEVELS]
    return render_template(
        "risk_matrix.html",
        impact_levels=IMPACT_LEVELS,
        probability_levels=prob_levels,
        is_mock=not Config.is_gitlab_configured(),
    )
...
def api_update_row(row_id):
    body = request.json or {}
    if "impact_level" in body:
        update_row_impact(row_id, body["impact_level"])
    if "prob_key" in body:
        update_row_probability(row_id, body["prob_key"], locked=True)
    return jsonify({"success": True})
...
def api_import_csv(matrix_id):
    ...
    # Enrichissement : récupération des scores "Impacts appli" depuis GitLab
    if Config.is_gitlab_configured() and Config.GITLAB_PROJECT_ID:
        new_rows = get_matrix_rows(matrix_id)
        for r in new_rows:
            iid = r.get("gitlab_iid", "").strip()
            if not iid or r.get("impact_description"):
                continue
            try:
                notes = fetch_issue_comments(Config.GITLAB_PROJECT_ID, iid)
                score = extract_impact_score_from_comments(notes)
                if score is not None:
                    update_row_impact_description(r["id"], str(score))
            except Exception:
                pass
