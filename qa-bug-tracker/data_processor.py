...
PROBABILITY_DICT = {key: {"key": key, "label": label, "value": value}
                    for key, label, value, _ in PROBABILITY_LEVELS}
...
def enrich_row(row, modules_data):
    ...
    Si prob_locked=True et prob_key est défini, on utilise la probabilité manuelle.
    """
    if row.get("prob_locked") and row.get("prob_key") in PROBABILITY_DICT:
        prob = PROBABILITY_DICT[row["prob_key"]]
    else:
