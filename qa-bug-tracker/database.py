...
def init_db():
    ...
    for col, definition in [
        ("weight",             "INTEGER DEFAULT NULL"),
        ("impact_description", "TEXT DEFAULT ''"),
        ("prob_key",           "TEXT DEFAULT NULL"),
        ("prob_locked",        "INTEGER DEFAULT 0"),
    ]:
        try:
...
def update_row_probability(row_id, prob_key, locked=True):
    conn = get_db()
    conn.execute(
        "UPDATE matrix_rows SET prob_key = ?, prob_locked = ? WHERE id = ?",
        (prob_key, 1 if locked else 0, row_id)
    )
    conn.commit()
    conn.close()


def update_row_impact_description(row_id, impact_description):
    conn = get_db()
    conn.execute(
        "UPDATE matrix_rows SET impact_description = ? WHERE id = ?",
        (impact_description, row_id)
    )
    conn.commit()
    conn.close()
